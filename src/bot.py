    ...
    def waiting(self, recast = False):
        ...
        try:
            ...
            while True:
                ...
                cast_pos, cast_conf = Detector.find_template(screenshot, self.template_loader.autoreel_template, CAST_THRESHOLD)

                # Show live debug overlay for waiting phase and save per-frame phase matches
                if DEBUG_MODE and self.debug is not None:
                    detections = {
                        'cast': {'pos': cast_pos, 'conf': cast_conf, 'template': self.template_loader.autoreel_template, 'color': (0, 255, 255)}
                    }
            ...
            while True:
                ...
                if DEBUG_MODE and self.debug is not None:
                    try:
                        ...
                        try:
                            ...
                            gray = None
                    # try:
                    #     self.debug.save_debug_images(screenshot_bgr, gray, cast_pos, bite_pos, None, None, None, frame_num)
                    # except Exception:
                    #     pass
                ...
                if not biting_done:
                    ...
                    if bite_pos is None:
                        ...
                        # no reeling because of auto-reel trait
